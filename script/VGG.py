...
class VGG16:
    ...
    # Wrong coding
    # def conv2d(self, x, W):
    #     height, width = x.get_shape().as_list()[1:-1]
    #     # kernel_dim = W.get_shape().as_list()[0]
    #     # w_pad = tf.pad(W, ([0, height-kernel_dim], [0, width-kernel_dim], [0, 0], [0, 0]), "CONSTANT")
    #     #
    #     # kernel = w_pad
    #     #
    #     # x_fft = x[..., tf.newaxis]
    #     # k_fft = kernel[tf.newaxis, ...]
    #     # ifft = tf.reduce_sum(x_fft * k_fft, axis=-2)
    #
    #     x_fft = tf.transpose(tf.spectral.rfft2d(tf.transpose(x, [0, 3, 1, 2])), [0,2,3,1])[..., tf.newaxis]
    #
    #     k_fft = tf.transpose(tf.spectral.rfft2d(tf.transpose(W, [2, 3, 0, 1]), [height, width]), [2,3,0,1])[tf.newaxis, ...]
    #
    #     fc = tf.transpose(tf.reduce_sum(tf.conj(x_fft) * k_fft, axis=-2), [0, 3, 1, 2])
    #
    #     ifft = tf.transpose(tf.real(tf.spectral.fft2d(fc))/height/width, [0, 2, 3, 1])
    #
...
if __name__ == '__main__':
    ...
    k=0.8
    x = tf.placeholder(tf.float32, [2, 112, 616, 3],name='x')
    keep_prob = tf.placeholder(tf.float32)
    VGG_test = VGG16()
    output = VGG_test.VGG16_conv(x, keep_prob, True, 'VGG_test')
    grd_local = tf.nn.max_pool(output, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME')
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        output = sess.run(grd_local, {x: sat_x, keep_prob: k})
        print(output.shape)
